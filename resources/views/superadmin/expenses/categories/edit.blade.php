...
@section('content')
<div class="container">
    <h1>Edit Expense Category: {{ $expenseCategory->name }}</h1>

    <div class="card">
        <div class="card-body">
            <form action="{{ route('superadmin.expense-categories.update', $expenseCategory->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <a href="{{ route('superadmin.expense-categories.index') }}" class="btn btn-secondary">Cancel</a>
            </form>
        </div>
    </div>
</div>
