...
@section('content')
<div class="container">
    <h1>Edit Expense Record #{{ $expense->id }}</h1>

    <div class="card">
        <div class="card-body">
            <form action="{{ route('superadmin.expenses.update', $expense->id) }}" method="POST" enctype="multipart/form-data"> {{-- Added enctype for file uploads --}}
                @csrf
                @method('PUT')
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="number" step="0.01" class="form-control @error('amount') is-invalid @enderror" id="amount" name="amount" value="{{ old('amount', $expense->amount) }}" required placeholder="0.00">
                        @error('amount')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="mb-3">
                    <label for="receipt" class="form-label">Upload New Receipt (Optional - JPG, PNG, PDF, max 2MB)</label>
                    ...
                    @if($expense->receipt_url)
                        <div class="mt-2">
                            Current Receipt: <a href="{{ Storage::url($expense->receipt_url) }}" target="_blank">View Receipt</a>
                            <small class="text-muted d-block">(Uploading a new file will replace the current one.)</small>
                        </div>
                    @endif
                </div>
